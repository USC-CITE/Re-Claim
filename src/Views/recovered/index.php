...
<head>
  ...
  <meta name="description" content="Find already recovered items.">
  ...
  <meta property="og:title" content="Recovered Items — WVSU Re:Claim">
  <meta property="og:description" content="Find already recovered items.">
  <meta property="og:url" content="https://reclaim.wvsu-usc.org/recovered">
  ...
  <meta name="twitter:title" content="Recovered Items - WVSU Re:Claim">
  <meta name="twitter:description" content="Find already recovered items.">
  <meta name="twitter:image" content="https://reclaim.wvsu-usc.org/assets/Re_Claim Logo Icon.svg">
  <link rel="canonical" href="https://reclaim.wvsu-usc.org/recovered">
  ...
  <title>Recovered Items — WVSU Re:Claim</title>
...
<main class="mx-auto max-w-[1327px] px-4 py-8 sm:px-6 sm:py-10">
  <hgroup class="mb-8 space-y-2 text-center">
        <h1 class="text-center text-display-md font-bold text-black">Recovered Items</h1>
  </hgroup>

  <!-- SEARCH AND FILTER -->
  <section class="mb-8 flex justify-center">
    <form class="flex w-full max-w-[575px] items-center justify-center gap-3" role="search">
      <label for="recovered-search" class="sr-only">Search recovered items</label>
  ...
  <nav aria-label="Listing type" class="mb-10 flex justify-center">
    <ul class="flex flex-wrap items-center justify-center gap-x-6 gap-y-3 text-md font-semibold sm:gap-8">
      ...
      <li>
        <a href="/found" class="border-b-2 border-transparent pb-1 text-secondary transition-colors hover:text-secondary">
          Found Items
        </a>
      </li>
      <li>
        <a href="/recovered" class="border-b-2 border-primary-500 pb-1 text-primary-700">
          Recovered Items
        </a>
      </li>
    </ul>
  ...
  <section class="mx-auto mb-8 hidden w-full max-w-[841px] rounded-[32px] bg-white p-8 shadow-[0_4px_16px_0_rgba(0,0,0,0.20)]" data-filter-panel>
    <div class="grid gap-6 md:grid-cols-3">
      <label class="flex flex-col gap-2 text-lg font-semibold text-black">
        Type
        <select id="recovered-type-filter" class="h-10 w-full rounded-[8px] border border-white-700 bg-white px-4 text-sm font-normal text-black">
          <option value="">All types</option>
          <option value="lost">Recovered (Lost)</option>
          <option value="found">Returned (Found)</option>
        </select>
      </label>
      <label class="flex flex-col gap-2 text-lg font-semibold text-black">
        Location
        <select id="recovered-location-filter" class="h-10 w-full rounded-[8px] border border-white-700 bg-white px-4 text-sm font-normal text-black">
          <option value="">All locations</option>
        </select>
      </label>
      <label class="flex flex-col gap-2 text-lg font-semibold text-black">
        Category
        <select id="recovered-category-filter" class="h-10 w-full rounded-[8px] border border-white-700 bg-white px-4 text-sm font-normal text-black">
  ...
  <?php if (empty($items)): ?>
    <p class="text-center py-10">No items have been recovered yet.</p>
  <?php else: ?>
    <section class="flex flex-wrap justify-center gap-6" data-listing-grid>
      <?php foreach ($items as $item): ?>
        <article class="item-card flex h-full w-full max-w-[405px] min-w-0 flex-col items-start gap-4 overflow-hidden rounded-[32px] border border-[#d9d9d9] bg-white px-4 py-5 shadow-[0_4px_16px_0_rgba(0,0,0,0.20)] sm:px-[22px] sm:py-6" 
                 data-item-status="Recovered" 
                 data-item-type="<?= htmlspecialchars($item['item_type']) ?>"
                 data-item-location="<?= htmlspecialchars($item['location']) ?>" 
                 data-item-categories="<?= htmlspecialchars(implode('|', $item['categories'] ?? [])) ?>">
              ...
              <div class="min-w-0 flex-1">
                ...
                <?php
                  ...
                  if (!empty($item['event_date'])) {
                    try {
                      $dateLabel = (new DateTime($item['event_date']))->format('F j, Y \a\t g:i A');
                    } catch (Exception $e) {
                      $dateLabel = (string) $item['event_date'];
                    }
                  }
                ?>
          ...
          <?php if (!empty($item['image_url'])): ?>
            <img src="<?= htmlspecialchars($item['image_url']) ?>" alt="Recovered item image" class="h-auto max-h-[420px] w-full rounded-2xl object-cover sm:h-[260.188px] sm:max-w-[362px] sm:self-center">
          <?php else: ?>
    ...
    <p class="hidden py-10 text-center text-md text-secondary" data-empty-search-state>No recovered items match your search.</p>
  <?php endif; ?>
</main>
