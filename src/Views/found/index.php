    ...
    <section class="mx-auto mb-8 hidden w-full max-w-[841px] rounded-[32px] bg-white p-8 shadow-[0_4px_16px_0_rgba(0,0,0,0.20)]" data-filter-panel>
        <div class="grid gap-6 md:grid-cols-2">
            <label class="flex flex-col gap-2 text-lg font-semibold text-black">
                Location
                <select id="found-location-filter" class="h-10 w-full rounded-[8px] border border-white-700 bg-white px-4 text-sm font-normal text-black">
                    <option value="">All locations</option>
                </select>
            </label>
            <label class="flex flex-col gap-2 text-lg font-semibold text-black">
                Category
                <select id="found-category-filter" class="h-10 w-full rounded-[8px] border border-white-700 bg-white px-4 text-sm font-normal text-black">
            ...
            <?php foreach ($foundItems as $item): ?>
                <article class="item-card flex h-full w-full max-w-[405px] min-w-0 flex-col items-start gap-4 overflow-hidden rounded-[32px] border border-[#d9d9d9] bg-white px-4 py-5 shadow-[0_4px_16px_0_rgba(0,0,0,0.20)] sm:px-[22px] sm:py-6" data-item-status="<?= htmlspecialchars((string)($item['status'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" data-item-location="<?= htmlspecialchars((string)($item['location'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" data-item-categories="<?= htmlspecialchars(implode('|', $item['categories'] ?? []), ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <?php if ($item['image_url']): ?>
                        <img src="<?= htmlspecialchars($item['image_url']) ?>" alt="<?= htmlspecialchars($item['title']) ?>" class="h-auto max-h-[420px] w-full rounded-2xl  object-contain sm:h-[260.188px] sm:max-w-[362px] sm:self-center">
                    <?php else: ?>
                    ...
                    <footer class="mt-auto flex w-full flex-wrap justify-end gap-3 pt-2">
                        <?php if (($item['status'] ?? '') !== 'Recovered'): ?>
                            <button class="ml-auto inline-flex w-full items-center justify-center gap-[10px] self-center rounded-[16px] bg-primary-500 px-6 py-3 text-md font-semibold text-white-500 transition-colors hover:bg-primary-600 sm:max-w-[362px]" 
