...
<header class="w-full bg-white shadow-md relative z-50">
    ...
    <nav
        class="mx-auto flex w-full max-w-[1327px] flex-col px-4 py-4 md:flex-row md:items-center md:justify-between sm:px-6">
        ...
        <div class="flex w-full shrink-0 items-center justify-center md:w-auto md:justify-start relative">
            ...
            <button id="mobile-menu-btn"
                class="absolute right-0 md:static md:block md:hidden text-gray-700 hover:text-blue-600 focus:outline-none">
        ...
        <div id="mobile-dropdown" class="absolute left-0 right-0 top-full hidden w-full flex-col gap-6 border-t border-gray-100 bg-white px-4 py-6 shadow-lg md:contents md:border-0 md:shadow-none md:p-0">
            ...
            <ul
                class="flex w-full flex-col md:flex-row flex-wrap items-center justify-center gap-x-4 gap-y-4 border-t border-gray-200 pt-5 text-center font-medium text-gray-700 md:ml-6 md:w-auto md:justify-end md:border-0 md:pt-0 md:gap-y-3">
                <!-- Ternary Operator to check if user is logged in display profile and logout if not register-->
                <?php if (!isset($_SESSION['user_id'])): ?>
                    <li><a href='/register' class='whitespace-nowrap hover:text-blue-600 transition'>Sign up</a></li>
                <?php else: ?>
                    <li>
